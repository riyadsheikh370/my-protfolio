function blackhole(element) {
  ...
  function getParticleGradient(alpha = 1) {
    const colors = [
      "#9c2241",
      "#c7495f",
      "#db904f",
  ...
  class Star {
    constructor() {
      ...
      this.color = getParticleGradient(1 - this.orbital / 255);
  ...
  const animate_up = document.querySelector(".animate-up");
  const animate_left = document.querySelector(".animate-left");
  const animate_right = document.querySelector(".animate-right");
  const intro = document.getElementById("blackholeIntro");
  const myBoxs = document.querySelector(".myBoxs");
  ...
  if (!hasEntered) {
      ...
      centerHover.addEventListener("click", function () {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            myBox.classList.remove("opacity-0", "pointer-none");
            myBox.classList.add("start-animation");
            animate_up.classList.add("animate-slide-up-fade");
            animate_left.classList.add("animate-slide-left-fade");
            animate_right.classList.add("animate-slide-right-fade");

            sessionStorage.setItem("hasEntered", "true");
          }, 400); // 0.4 seconds to return slowly
        }, 2000); // 2 seconds of expansion experience
      });
    ...
    if (centerHover) centerHover.style.display = "none";
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        animate_left.classList.add("animate-slide-left-fade");
        animate_right.classList.add("animate-slide-right-fade");
      }, 200); // 0.4 seconds to return slowly
    }, 300);
  }

  // --- Auto Blackhole Intro for all non-index pages ---
  const autoIntroPages = [
    "portfolio.html",
    "blog.html",
    "resume.html",
    "contact.html",
  ...
  if (autoIntroPages.includes(currentPage)) {
    window.addEventListener("load", () => {
      console.log("Auto blackhole intro running for:", currentPage);

      // Trigger expansion sequence
      expanse = true;
      collapse = false;
      returning = false;
      ...
      setTimeout(() => {
        ...
        if (intro) {
          ...
          setTimeout(() => {
            ...
            if (myBoxs) {
              myBoxs.classList.remove("opacity-0");
              myBoxs.classList.add("start-animation");

              if (animate_left || animate_right || animate_up) {
                animate_up.classList.add("animate-slide-up-fade");
                animate_left.classList.add("animate-slide-left-fade");
                animate_right.classList.add("animate-slide-right-fade");
              }
